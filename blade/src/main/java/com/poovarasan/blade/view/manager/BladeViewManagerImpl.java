...
/**
 * BladeViewManagerImpl
 *
 * @author Poovarasan Vasudevan
 */
public class BladeViewManagerImpl implements BladeViewManager {

    private static final String TAG = "BladeViewManagerImpl";
    private View view;
        ...
        if (view instanceof ViewGroup) {
            if (dataPathForChildren != null) {
                ...
                for (int index = 0; index < childCount; index++) {
                    ...
                    if (child instanceof BladeView) {
                        ((BladeView) child).getViewManager().update(dataContext.getData());
                    }
                }
            }
        }
    ...
    private void updateChildrenFromData() {
        ...
        if (childCount > dataList.size()) {
            while (childCount > dataList.size()) {
                ...
                if (child instanceof BladeView) {
                    ((BladeView) child).getViewManager().destroy();
                }
    ...
    private void handleBinding(Binding binding) {
        if (binding.hasRegEx()) {
            layoutBuilder.handleAttribute(layoutHandler, (BladeView) view, binding.getAttributeKey(), new JsonPrimitive(binding.getAttributeValue()));
        } else {
            ...
            layoutBuilder.handleAttribute(layoutHandler, (BladeView) view, binding.getAttributeKey(), dataValue);
        }
    }
        ...
        if (view instanceof ViewGroup) {
            ...
            for (int index = 0; index < childCount; index++) {
                ...
                if (child instanceof BladeView) {
                    ...
                    ((BladeViewManagerImpl) ((BladeView) child).getViewManager()).update(aliasedDataPath);
                }
            }
        }
